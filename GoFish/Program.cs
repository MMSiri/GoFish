...
namespace GoFish
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Console.Write("Enter the number of COM opponents: ");
            int opponentCount;
            ...
            while (!gameController.GameOver)
            {
                while (!gameController.GameOver)
                {
                    ...
                    foreach (var card in gameController.HumanPlayer.Hand.OrderBy(card => card.Suit).OrderBy(card => card.Value)) Console.WriteLine(card);
                ...
                Console.WriteLine("Press 'Q' to quit or press any other key to start a new game.");
                if (Console.ReadKey(true).KeyChar.ToString().ToUpper() != "Q") gameController.NewGame();
            }
        }

        /// <summary>
        /// GameController needed to manage the game.
        /// </summary>
        ...
        static Values PromptForAValue()
        {
            ...
            while (true)
            {
                if (Enum.TryParse(typeof(Values), Console.ReadLine(), out var value) && handValues.Contains((Values)value)) return (Values)value;
                else Console.WriteLine("Please enter a value in your hand.");
            }
        }
        ...
        static Player PromptForAnOpponent()
        {
            ...
            for (int i = 1; i < opponents.Count; i++) Console.WriteLine($"{i}. {opponents[i - 1]}");
            Console.Write("Who do you want to ask for a card? ");
            while (true)
            {
                if (int.TryParse(Console.ReadLine(), out int selection) && selection >= 1 && selection <= opponents.Count()) return opponents[selection - 1];
                else Console.Write($"Please enter a number from 1 to {opponents.Count()}: ");
            }
        }
